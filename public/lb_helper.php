...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = '3CE5CBAD';
		$this->api_url = 'https://scriptscode7.com/license/';
		$this->api_key = '246F395593E52B2CC7E7';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["f9f345e8ba7aa39"])){
				$_SESSION["f9f345e8ba7aa39"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["f9f345e8ba7aa39"])){
				$get_data = $this->call_api(
					'POST',
